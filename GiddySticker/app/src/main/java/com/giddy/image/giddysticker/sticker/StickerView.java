package com.giddy.image.giddysticker.sticker;
...
/**
 * Created by Binh NK on 2/5/2017.
 */

public abstract class StickerView extends FrameLayout {

    public static final String TAG = "com.kabin.sticker.view";
    ...
    private void init(Context context) {
        ...
        this.iv_ok.setImageResource(R.drawable.success);
    ...
    public void makeStaticSticker() {
        if (iv_border.getVisibility() == VISIBLE) {
            iv_border.setVisibility(GONE);
        }
        if (iv_scale.getVisibility() == VISIBLE) {
            iv_scale.setVisibility(GONE);
        }
        if (iv_delete.getVisibility() == VISIBLE) {
            iv_delete.setVisibility(GONE);
        }
        if (iv_flip.getVisibility() == VISIBLE) {
            iv_flip.setVisibility(GONE);
        }
        if (iv_ok.getVisibility() == VISIBLE) {
            iv_ok.setVisibility(GONE);
        }
    }
    public void makeDynamicSticker() {
        if (iv_border.getVisibility() == GONE) {
            iv_border.setVisibility(VISIBLE);
        }
        if (iv_scale.getVisibility() == GONE) {
            iv_scale.setVisibility(VISIBLE);
        }
        if (iv_delete.getVisibility() == GONE) {
            iv_delete.setVisibility(VISIBLE);
        }
        if (iv_flip.getVisibility() == GONE) {
            iv_flip.setVisibility(VISIBLE);
        }
        if (iv_ok.getVisibility() == GONE) {
